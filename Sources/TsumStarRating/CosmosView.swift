...
@IBDesignable open class CosmosView: UIView {
    ...
    private var isInitialTouch = true
    private var previousRatingForStarTap: Double = 0
    ...
    func onDidTouch(_ locationX: CGFloat) {
        ...
        if isInitialTouch {
            previousRatingForStarTap = calculatedTouchRating
        }
    }
    
    func onDidEndTouch(_ locationX: CGFloat) {
        var calculatedTouchRating = CosmosTouch.touchRating(locationX, settings: settings)
        if !isInitialTouch,
           calculatedTouchRating == previousRatingForStarTap {
            calculatedTouchRating -= 1
        }
